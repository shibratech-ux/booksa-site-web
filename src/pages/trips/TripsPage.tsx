...
function TripEmptyState() {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center px-7 pb-5 text-center">
      <div className="relative flex h-[160px] w-[190px] items-center justify-center">
        <span className="select-none text-[92px]" aria-hidden="true">🗺️</span>
        <ThreeDIcon
          name="pastTrips"
          sourceSize={200}
      ...
      <h1 className="mt-2 text-[20px] font-semibold tracking-[-0.035em]">Map out your next trip</h1>
      <p className="mt-3 max-w-[280px] text-[12px] leading-[1.45] text-slate-500">
        After you book a trip, experience, or service, come back here to see details, explore the map, and save places to visit.
      </p>
      <button
        type="button"
        onClick={() => navigate(ROUTES.home)}
        className="mt-5 h-11 rounded-[11px] bg-[#e9145f] px-6 text-[13px] font-semibold text-white shadow-sm transition active:scale-95"
      >
        Get started
      </button>
    </div>
  ...
  return (
    <main className="h-[100dvh] overflow-hidden bg-white text-slate-900">
      <div className="relative h-[calc(100dvh-66px)] sm:hidden">
        ...
        <section className="absolute inset-x-0 bottom-0 top-[310px] z-20 rounded-t-[24px] bg-white shadow-[0_-7px_24px_rgba(15,23,42,0.10)]">
          <div className="mx-auto mt-1.5 h-1 w-10 rounded-full bg-slate-300" />
          <TripEmptyState />
        </section>
      </div>
