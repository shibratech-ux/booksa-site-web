      ...
      <div className={`${galleryFrameClass} grid gap-1.5 lg:h-full lg:grid-cols-[1.35fr_0.9fr]`}>
        <div className="min-h-0 lg:h-full">
          <button type="button" onClick={onImageClick} className="block h-full w-full cursor-pointer">
            <img alt={`${label} vue principale`} className="h-auto min-h-[260px] w-full object-cover sm:min-h-[420px] lg:h-full" src={images[0]} />
          </button>
        </div>
        <div className="grid gap-1.5 lg:h-full">
          {images.slice(1).map((image, index) => (
            <div key={`${image}-${index}`} className="min-h-0 lg:h-1/2">
              <button type="button" onClick={onImageClick} className="block h-full w-full cursor-pointer">
                <img alt={`${label} détail ${index + 1}`} className="h-auto min-h-[130px] w-full object-cover sm:min-h-[209px] lg:h-full" src={image} />
              </button>
            </div>
          ))}
        </div>
    ...
    return (
      <div className={`${galleryFrameClass} grid gap-1.5 sm:grid-cols-2 lg:h-full`}>
        {images.map((image, index) => (
          <div key={`${image}-${index}`} className="min-h-0 lg:h-1/2">
            <button type="button" onClick={onImageClick} className="block h-full w-full cursor-pointer">
              <img alt={`${label} détail ${index + 1}`} className="h-auto min-h-[130px] w-full object-cover sm:min-h-[240px] lg:h-full" src={image} />
            </button>
          </div>
        ))}
      </div>
  ...
  if (images.length === 5) {
    return (
      <div className={`${galleryFrameClass} grid gap-1.5 lg:h-full lg:grid-cols-[1.08fr_0.92fr]`}>
        <div className="min-h-0 lg:h-full">
          <button type="button" onClick={onImageClick} className="block h-full w-full cursor-pointer">
            <img alt={`${label} vue principale`} className="h-auto min-h-[260px] w-full object-cover sm:min-h-[485px] lg:h-full" src={images[0]} />
          </button>
        </div>
    ...
    return (
      <div className={`${galleryFrameClass} grid gap-1.5 sm:grid-cols-2 xl:grid-cols-3 lg:h-full`}>
        {images.slice(0, 6).map((image, index) => (
          <div key={`${image}-${index}`} className="min-h-0 lg:h-1/2">
            <button type="button" onClick={onImageClick} className="block h-full w-full cursor-pointer">
              <img alt={`${label} détail ${index + 1}`} className="h-auto min-h-[130px] w-full object-cover sm:min-h-[220px] lg:h-full" src={image} />
            </button>
          </div>
        ))}
      </div>
  ...
  return (
    ...
    >
      <div className="space-y-3 lg:sticky lg:top-8 lg:self-start">
        <h2 className="text-[20px] font-medium tracking-[-0.03em] text-gray-900 sm:text-[18px]">
          {section.label}
        </h2>
        <p className="max-w-md text-[12px] font-normal leading-5 text-gray-500 sm:text-[14px] sm:leading-6">
          {section.description}
        </p>
      </div>
...
function PhotoTourMobileCard({
  ...
  return (
    <motion.button
      ...
      className="group relative overflow-hidden rounded-[24px] bg-slate-100 text-left shadow-[0_16px_38px_rgba(15,23,42,0.12)] ring-1 ring-black/5"
      whileTap={{ scale: 0.98 }}
    >
      <div
        ref={mobileCarouselRef}
      ...
      >
        {section.images.map((image, index) => (
          <div key={`${section.id}-${image}-${index}`} className="h-full w-full shrink-0 snap-center">
            <img alt={`${section.label} image ${index + 1}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" src={image} />
          </div>
        ))}
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 p-3">
        ...
        <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-white/80">{section.description}</p>
      </div>

      {section.images.length > 1 ? (
        <div className="absolute right-3 top-3 rounded-full bg-black/45 px-2.5 py-1 text-[10px] font-medium text-white backdrop-blur-sm">
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <main className="mx-auto max-w-[1440px] px-4 pb-16 pt-4 sm:px-8 sm:pb-20 sm:pt-8 lg:px-14">
        ...
        >
          <h1 className="text-[20px] font-medium tracking-[-0.04em] text-gray-900 sm:text-[22px]">
            Visite photo
          </h1>
          <p className="mt-1 max-w-xl text-[12px] leading-5 text-gray-500 sm:mt-3 sm:max-w-2xl sm:text-[13px] sm:leading-4">
            {galleryTitle}
        ...
        >
          {sections.map((section) => (
            ...
            >
              <div className="overflow-hidden rounded-[18px] bg-slate-100 shadow-[0_12px_30px_rgba(15,23,42,0.08)] ring-1 ring-black/5">
                <img
                  alt={section.label}
              ...
              <p className="mt-1.5 text-[11px] font-normal text-gray-900 sm:mt-2 sm:text-[13px]">{section.label}</p>
            </motion.button>
          ))}
        </motion.div>
