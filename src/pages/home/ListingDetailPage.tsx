...
function DetailStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[20px] border border-slate-200 bg-white px-4 py-3 shadow-[0_10px_25px_rgba(15,23,42,0.05)]">
...
function SectionHeading({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="space-y-1">
      <h2 className="text-[21px] font-medium tracking-tight text-gray-900">{title}</h2>
      {subtitle ? <p className="text-[12px] leading-6 text-gray-900">{subtitle}</p> : null}
    </div>
  );
}
...
function MobileGalleryHero({
  ...
  return (
    <div className="-mx-4 -my-5 md:hidden sm:-mx-6 sm:-my-8">
      <div className="relative overflow-hidden">
        ...
        <div className="absolute left-4 right-4 top-4 flex items-center justify-between">
          <button
            type="button"
            onClick={onBack}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/85 text-gray-900 shadow-[0_10px_22px_rgba(15,23,42,0.16)] backdrop-blur"
            aria-label="Retour"
          >
      ...
      <div className="-mt-5 rounded-t-[24px] bg-white px-5 pt-10 text-center shadow-[0_-8px_32px_rgba(15,23,42,0.08)]">
        <h1 className="text-[22px] font-medium tracking-tight text-gray-900">
      ...
      <div className="mx-auto max-w-lg">
        <div className="flex items-center justify-between border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-4 shadow-[0_-12px_32px_rgba(15,23,42,0.12)]">
          ...
          <button
            type="button"
            onClick={onReserve}
            className="min-w-[128px] rounded-full bg-[var(--color-primary-500)] px-8 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-[var(--color-primary-600)] active:scale-95"
          >
            Réserver
          </button>
        </div>
...
function DateRangePicker({
  ...
  const headingText = range.start && range.end ? `${nights} nuits au Cap` : 'Sélectionner des dates';
      ...
      <div className="overflow-hidden px-0 py-0">
        <div className="px-0 sm:px-0">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              <h3 className="text-[18px] font-medium tracking-[-0.03em] text-gray-900 sm:text-[18px]">
                {headingText}
              </h3>
              <p className="mt-1 text-[13px] text-gray-500 sm:text-[12px]">
            ...
            <div className="grid flex-1 grid-cols-1 px-4 sm:px-8 lg:grid-cols-2">
              {months.map((month, index) => (
                <h4
                  ...
                  className={`text-center text-[13px] font-semibold tracking-tight text-gray-900 sm:text-[14px] ${index === 1 ? 'hidden lg:block' : ''}`}
                >
                ...
                <div key={month.format('YYYY-MM')} className={`space-y-5 ${index === 1 ? 'hidden lg:block' : ''}`}>
                  <div className="grid grid-cols-7 gap-y-1 text-center">
                    {calendarWeekdays.map((weekday) => (
                      <div key={`${month.format('YYYY-MM')}-${weekday}`} className="pb-3 text-[10px] font-medium text-gray-500">
                        {weekday}
                      </div>
                    ))}
                    ...
                    {cells.map((date, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={[
                            'relative flex h-10 w-10 items-center justify-center text-[11px] font-medium transition',
                            isSelected
                              ? 'z-10 text-white'
                              : inRange
                                ? 'text-gray-900'
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <div className="min-w-0 pr-2">
        ...
        <p className={`text-[11px] leading-5 ${note ? 'underline underline-offset-2' : 'text-gray-600'}`}>
          {subtitle}
        </p>
        {note ? <p className="mt-1 text-[8px] leading-5 text-gray-600">{note}</p> : null}
      </div>
...
function GuestPickerDialog({
  ...
  return (
    <div className="absolute left-1/2 top-full z-40 mt-2 w-[min(100vw-2rem,360px)] -translate-x-1/2">
      <div className="rounded-[18px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.18)]">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-500">Voyageurs</p>
...
function AmenitiesDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-[2px]">
      ...
      <div className="modal-animation relative z-10 w-full max-w-[760px] overflow-hidden rounded-[32px] bg-white shadow-[0_28px_70px_rgba(15,23,42,0.22)]">
        ...
        <div className="max-h-[74vh] overflow-y-auto px-6 pb-8 pt-4 sm:px-8">
          <h2 className="text-[21px] font-medium tracking-[-0.03em] text-gray-900">
          ...
          <div className="mt-8 space-y-9">
            {amenityGroups.map((group, groupIndex) => (
              <section key={group.title} className="space-y-4">
                <h3 className="text-[18px] font-semibold tracking-tight text-gray-900">{group.title}</h3>
                  ...
                  {group.items.map((item, itemIndex) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-[16px] font-medium text-gray-800">{item.label}</p>
                      </div>
                    );
                  })}
                </div>
...
function GuestReviewsDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/45 px-4 py-6 backdrop-blur-[2px]">
      ...
      <div className="relative z-10 mt-2 flex h-[min(88vh,820px)] w-full max-w-[820px] flex-col overflow-hidden rounded-[32px] bg-white shadow-[0_28px_80px_rgba(15,23,42,0.25)]">
        <div className="flex items-start justify-between px-6 pb-2 pt-6 sm:px-8">
          <div className="space-y-2">
            <h2 className="text-[21px] font-medium tracking-[-0.04em] text-gray-900">113 avis de voyageurs</h2>
          ...
          <div className="space-y-8">
            {reviewItems.map((review, index) => (
              <article key={`${review.name}-${review.location}-${index}`} className="border-b border-slate-200 pb-8 last:border-b-0">
                ...
                <p className="mt-4 max-w-3xl text-[15px] leading-7 text-gray-700">{review.text}</p>
              </article>
            ))}
          </div>
        </div>
...
function BookingCard({
  ...
  const checkInLabel = range.start ? range.start.format('M/D/YYYY') : 'Sélectionner des dates';
  const checkOutLabel = range.end ? range.end.format('M/D/YYYY') : 'Sélectionner des dates';
  ...
  return (
    <div className="relative mt-10 rounded-[16px] border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
      <div className="mt-4">
        <p className="text-[18px] font-semibold tracking-tight text-gray-900">
          <span>{getReservationPrice(price, nights)}</span>
          <span className="px-2 text-sm font-normal text-gray-600">pour {nights} nuits</span>
        </p>
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-[18px] border border-slate-300">
        <div className="grid grid-cols-2 divide-x divide-slate-300 border-b border-slate-300">
          <div className="px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-gray-500">Arrivée</p>
            <p className="mt-1 text-xs font-medium text-gray-900">{checkInLabel}</p>
          </div>
          <div className="px-4 py-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-500">Départ</p>
        ...
        >
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-gray-500">Voyageurs</p>
            <p className="mt-1 text-xs font-medium text-gray-900">
              {guestTotal} {guestTotal === 1 ? 'voyageur' : 'voyageurs'}
            </p>
          </div>
      ...
      <div className="mt-3 rounded-full bg-slate-100 px-4 py-2 text-center text-[12px] font-medium text-gray-700">
        Annulation gratuite
      </div>
...
function RoomCard({ listing, nights }: { listing: Listing; nights: number }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.05)] md:flex-row">
      <div className="md:w-[240px]">
        <img alt={listing.location} className="h-full w-full object-cover md:min-h-[170px]" src={roomImage} />
      </div>
      <div className="flex flex-1 flex-col justify-center gap-1 p-5">
        ...
        <p className="text-sm font-semibold text-gray-900">
          ...
          <span className="font-normal text-gray-600">pour {nights} nuits</span>
        </p>
      </div>
    </div>
  );
    ...
    <section className="pt-8">
      <div className="py-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-[16px] font-medium tracking-tight text-gray-900">Hébergé par {hostName}</h3>
        ...
        <div className="space-y-6">
          {hostHighlights.map(({ icon: Icon, title, description }) => (
            <div key={title} className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <h4 className="text-[14px] font-medium tracking-tight text-gray-900">{title}</h4>
                <p className="mt-1 text-[14px] font-normal leading-4 text-gray-500">{description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="mx-auto max-w-[1500px] px-0 py-5 sm:px-6 sm:py-8 lg:px-44">
        ...
        <div id="photos" className="space-y-5">
          <div className="hidden items-start justify-between gap-4 lg:flex">
            <div>
              <h1 className="text-[22px] font-medium tracking-tight text-gray-900 sm:text-[24px]">
                {listing.location}
              </h1>
          ...
          <div className="hidden gap-1.5 overflow-hidden rounded-[28px] md:grid">
            <div className="grid gap-1.5 lg:grid-cols-[2fr_1fr_1fr]">
              ...
              type="button" className="group cursor-pointer relative overflow-hidden">
                <img alt={listing.location} className="h-full min-h-[390px] w-full object-cover transition duration-500 group-hover:scale-[1.03]" src={heroImage} />
              ...
              <div className="grid gap-1.5">
                {secondaryImages.slice(2, 4).map((image, index) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/12 via-transparent to-transparent" />
                    {index === 1 ? (
                      <span className="absolute bottom-4 right-4 rounded-sm bg-white px-4 py-2 text-xs font-medium text-gray-900 shadow-[0_12px_28px_rgba(15,23,42,0.18)] transition group-hover:-translate-y-0.5 group-hover:shadow-[0_16px_32px_rgba(15,23,42,0.22)]">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="space-y-8">
            <div className="hidden gap-6 md:grid xl:grid-cols-[minmax(0,1fr)_105px]">
              <div className="w-full space-y-5">
                ...
                <p className="max-w-3xl text-sm sm:text-[16px] font-normal leading-6 text-gray-600">
            ...
            <section id="reviews" className="space-y-5 border-t border-slate-200 pt-8">
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <h2 className="text-[21px] font-medium tracking-tight text-gray-900">
            ...
            <section id="location" className="space-y-5 border-t border-slate-200 pt-8">
              ...
              <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
          ...
          <aside className="hidden space-y-4 lg:sticky lg:top-24 lg:block lg:self-start">
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white p-5 text-sm text-gray-600 shadow-[0_10px_25px_rgba(15,23,42,0.04)]">
...
export  function MeetYourHost() {
  return (
    <section className="w-full border-t border-gray-200 bg-white px-6 py-10">
        <h2 className="mb-6 text-[21px] font-medium text-neutral-900">
