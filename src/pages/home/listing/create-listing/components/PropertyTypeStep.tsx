          ...
          {listingCategories.map(({ id, label, shortDescription, icon: Icon, informationSections }) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-[132px] flex-col items-start rounded-[var(--radius-lg)] border bg-[var(--color-surface)] p-4 text-left transition hover:-translate-y-0.5 hover:border-[var(--color-text-primary)] hover:shadow-[var(--shadow-sm)] sm:min-h-[180px] sm:p-5 md:min-h-[220px] ${
                  selected
              ...
              >
                ...
                <span className="mt-2 flex flex-wrap gap-1.5">
                  {informationSections.map(({ id: informationId, title }) => (
                    <span
                      key={informationId}
                      className="rounded-full bg-[var(--color-surface-muted)] px-2 py-1 text-xs font-medium text-[var(--color-text-primary)]"
                    >
                      {title}
                    </span>
                  ))}
                ...
                {selectedCategory.options.map(({ id, label, icon: Icon }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-[96px] flex-col items-start justify-center rounded-[var(--radius-md)] border bg-[var(--color-surface)] px-4 py-3 text-left transition hover:border-[var(--color-text-primary)] ${
                        selected
                          ? 'border-[var(--color-text-primary)] ring-2 ring-[var(--color-text-primary)]'
                          : 'border-[var(--color-border)]'
                      }`}
