...
function PrimaryButton({ children }: { children: React.ReactNode }) {
  return (
    <button
      type="button"
      className="mt-6 min-h-12 rounded-[var(--radius-md)] bg-[var(--color-text-primary)] px-6 font-semibold text-[var(--color-surface)] transition hover:opacity-85"
    >
      {children}
    </button>
  );
  ...
  return (
    <div>
      ...
      <section className="py-16">
        ...
        <article className="mt-7 rounded-[var(--radius-lg)] border border-[var(--color-border)] p-6">
...
function PayoutsTabContent() {
  return (
    <div>
      <section className="pt-8">
        <h3 className="text-[23.52px] font-semibold tracking-[-0.035em]">How you’ll get paid</h3>
      ...
      <section className="mt-14 rounded-[var(--radius-lg)] border border-[var(--color-border)] p-6">
    ...
    <div className="pt-8">
      ...
      <div className="mt-8 space-y-10">
        <label className="flex cursor-pointer items-start gap-4">
          ...
          <span>
            <span className="font-semibold">Single fee <span className="ml-1 rounded-sm bg-[var(--color-surface-muted)] px-2 py-1 text-[9.24px]">RECOMMENDED</span></span>
      ...
      <div className="mt-8 flex gap-4 border-t border-[var(--color-border)] pt-6">
        <button type="button" onClick={() => void saveFee()} disabled={selectedFee === savedFee || isSaving} className="min-h-12 rounded-[var(--radius-md)] bg-[var(--color-text-primary)] px-9 font-semibold text-[var(--color-surface)] disabled:cursor-default disabled:opacity-30">{isSaving ? 'Saving…' : 'Save'}</button>
        <button type="button" onClick={() => setSelectedFee(savedFee)} className="min-h-12 rounded-[var(--radius-md)] bg-[var(--color-surface-muted)] px-9 font-semibold">Cancel</button>
      </div>
    </div>
  );
}
...
function DonationsTabContent() {
  ...
  return (
    <div className="pt-8">
      <article className="flex gap-4 rounded-[var(--radius-lg)] border border-[var(--color-border)] p-5">
        <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-blue-500 text-white"><FiBell aria-hidden="true" /></span>
      ...
      <section className="pt-10">
        <h3 className="text-[25.2px] font-semibold tracking-[-0.04em]">Donate to Booksa.org</h3>
        ...
        <div className="mt-5 flex flex-wrap gap-3">{['1%', '3%', '5%', 'Other'].map((amount) => <button key={amount} type="button" disabled className="rounded-full border border-[var(--color-border)] px-7 py-3 text-[var(--color-text-secondary)] opacity-60">{amount}</button>)}</div>
      </section>
      ...
      <section className="py-10">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          {[
            ...
            <article key={title} className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border)] shadow-sm"><ShimmerImage src={image} alt="" className="h-48 w-full object-cover" /><div className="p-4"><h4 className="font-semibold">{title}</h4><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{location}</p></div></article>
          ))}
        </div>
      </section>
    </div>
...
export function PaymentsContent({ initialServiceFee, onServiceFeeSaved }: { initialServiceFee?: string; onServiceFeeSaved?: (fee: ServiceFee) => void }) {
  ...
  return (
    <div className="mx-auto max-w-[760px]">
      <h2 className="text-[23.52px] font-semibold tracking-[-0.035em] sm:text-[25.2px]">Payments</h2>
      <div className="mt-6 flex overflow-x-auto border-b border-[var(--color-border)]" role="tablist" aria-label="Payment settings">
        {tabs.map(({ id, label }) => (
          <button key={id} type="button" role="tab" aria-selected={activeTab === id} onClick={() => setActiveTab(id)} className={`relative min-w-max px-4 pb-3 text-sm font-medium first:pl-0 ${activeTab === id ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)]'}`}>
            {label}{activeTab === id ? <motion.span layoutId="payments-tab-indicator" className="absolute inset-x-0 -bottom-px h-0.5 rounded-full bg-[var(--color-text-primary)]" /> : null}
          </button>
        ))}
      </div>
