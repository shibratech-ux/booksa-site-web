  ...
  return (
    ...
    >
      <PhotoTile
        ...
        className="aspect-[4/3] min-h-[198px] rounded-photo sm:aspect-auto sm:min-h-[288.2px] lg:h-[420.2px]"
      ...
      <div className="grid gap-3 max-[619px]:grid-cols-2 max-[619px]:gap-2 sm:grid-cols-2 lg:grid-rows-2">
        {visibleImages.slice(1).map((image) => (
          <PhotoTile
            ...
            alt={image.alt}
            className="aspect-[4/3] min-h-[118.8px] rounded-photo sm:aspect-auto lg:h-full"
            src={image.src}
          />
        ))}
        ...
        <button
          type="button"
          className="group relative min-h-[118.8px] overflow-hidden rounded-photo border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-sm)] sm:aspect-auto"
        >
          <ShimmerImage
          ...
          <span className="absolute bottom-4 right-4 inline-flex min-h-9 items-center gap-2 rounded-button bg-white px-4 py-2 text-xs font-semibold text-gray-900 shadow-[var(--shadow-sm)] transition group-hover:translate-y-[-1px]">
...
export function ListingShowcase() {
  return (
    <section className="space-y-5">
      <div className="hidden overflow-hidden rounded-photo min-[620px]:block">
